 ...
 * the inherited class EmployeeForm1Component.
 **/
@Component({ template: '' })
export class EmployeeForm1BaseComponent
  ...
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    this.store.dispatch(
      fromEmployee.initStateAction({
        ...this.params,
      })
    )
    this.store.dispatch(
      fromEmployee.initValueAction({ ...this.getActionPayload() })
    )
  ...
  updateDataInState(value, displayScreen = true) {
    super.updateDataInState(fromEmployee.updateDataAction, value, displayScreen)
  }
  /**
  ...
  /***
   * Executes the Button54477 action
   * @return {void}
   **/
  doButton54477(): void {
    this.store.dispatch(
      fromEmployee.button54477Action({ ...this.getActionPayload() })
    )
  }
}
