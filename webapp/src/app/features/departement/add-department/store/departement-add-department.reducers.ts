...
import * as fromDepartement from './departement-add-department.actions'
import * as fromStore from '@app/core/store'
import * as featureAdapter from '@core/store/feature-adapter'
/**
 * The basic State interface for the `add-department` component
 *
 */
export interface State extends EntityState<ScreenState<any>> {}
/**
 * The basic NGRX reducer adapter for the `add-department` component
 *
 */
...
>({})
/**
 * The default state's properties for the `add-department` component
 *
 */
...
const _reducer = createReducer(
  ...
  on(fromDepartement.initStateAction, (state, payload) =>
    adapter.addOne({ ...payload, loading: true }, state)
  ),
  on(fromDepartement.shareWidgetData, (state, payload) =>
    featureAdapter.shareWidgetData(state, payload, adapter)
  ),
  on(fromDepartement.initValueSuccessAction, (state, payload) =>
    featureAdapter.initValue<any>(state, payload, adapter)
  ),
  on(fromDepartement.initValueFailAction, (state, payload) =>
  ...
  on(fromDepartement.updateDataAction, (state, payload) =>
  ...
  on(fromDepartement.button42178Action, (state, payload) => {
  ...
  on(
    fromDepartement.button42178FailAction,
