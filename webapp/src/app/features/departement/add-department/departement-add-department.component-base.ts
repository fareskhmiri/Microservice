 ...
 * the inherited class DepartementAddDepartmentComponent.
 **/
@Component({ template: '' })
export class DepartementAddDepartmentBaseComponent
  ...
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    this.store.dispatch(
      fromDepartement.initStateAction({
        ...this.params,
      })
    )
    this.store.dispatch(
      fromDepartement.initValueAction({ ...this.getActionPayload() })
    )
  ...
  updateDataInState(value, displayScreen = true) {
    super.updateDataInState(
      fromDepartement.updateDataAction,
      value,
      displayScreen
    )
  }
  ...
  /**
   * Executes the Button42178 action
   * @return {void}
   */
  doButton42178(): void {
    if (this.form.valid) {
      this.store.dispatch(
        fromDepartement.button42178Action({ ...this.getActionPayload() })
      )
    } else {
