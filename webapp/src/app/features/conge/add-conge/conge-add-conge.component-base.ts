 ...
 * the inherited class CongeAddCongeComponent.
 **/
@Component({ template: '' })
export class CongeAddCongeBaseComponent
  ...
  implements OnInit
{
   ...
   */
  keys = `Date87740,Date87740092080`
  ...
  ngOnInit() {
    ...
    this.store.dispatch(
      fromConge.initStateAction({
        ...this.params,
      })
    )
    this.store.dispatch(
      fromConge.initValueAction({ ...this.getActionPayload() })
    )
    ...
    this.featureService.calendarClearValue()
  }
  ...
  updateDataInState(value, displayScreen = true) {
    super.updateDataInState(fromConge.updateDataAction, value, displayScreen)
  }
  /**
   ...
   * @return {void}
   */
  doButton35872039101(): void {
    if (this.form.valid) {
      this.store.dispatch(
        fromConge.button35872039101Action({ ...this.getActionPayload() })
      )
    } else {
  ...
  /**
   * Executes the Button35872 action
   * @return {void}
   */
  doButton35872(): void {
    if (this.form.valid) {
      this.store.dispatch(
        fromConge.button35872Action({ ...this.getActionPayload() })
      )
    } else {
      this.featureService.validateForm(this.form)
    }
  ...
  /**
   * Sets the focus to the Calendar component after closing or hiding the calendar dialog
   * @param element
   * @param value
   * @param isClosing
   */
  setDateFocus(element, value, isClosing = true) {
    this.featureService.setFocus(
      isClosing ? element.el : element.contentViewChild,
      value
    )
  }
}
